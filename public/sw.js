...
// cached shell. Only bites on hanging connections (extreme latency, captive
// portals) — a truly offline device rejects the fetch immediately and falls
// back with no delay. Generous enough that a merely-slow network still gets
...
self.addEventListener('fetch', (event) => {
  ...
  // HTML / app shell → network-first. Every open/reload asks the origin for
  // the freshest HTML, so a new deploy is live on the very next open — no
  // "quit twice" propagation lag, no update prompts. The cost is one small
  // (~7KB) HTML round-trip per open; the heavy startup JS still comes from
  // the permanent BUILD_CACHE, so repeat opens stay fast. The cached copy is
  // only a fallback: served when the fetch rejects (offline — instant) or
  // when the network hangs past NAV_NETWORK_TIMEOUT_MS. Falls back to the
