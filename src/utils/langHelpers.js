...
export const UI_TEXT = {
  zh: {
    ...
    installIosTip: '请点击底部「⋯」按钮，然后点击「分享」按钮，选择 View More，下滑，然后选择「添加到主屏幕」。',
    installIosChromeTip: '请点击右上角「分享」按钮，选择 View More，下滑，然后选择「添加到主屏幕」。',
    installAndroidTip: '请点击 Chrome 右上角的「⋮」菜单，选择「安装应用」或「添加到主屏幕」即可。\n\n如果 Chrome 说「已安装」但你找不到图标：长按桌面/抽屉里的旧 PlushieWord 图标删除，或在系统设置 → 应用里卸载，再回来试一次。',
    installDesktopTip: '从浏览器菜单装：\n\n1. 点击右上角的「⋮」三点菜单\n2. 找到「投放、保存和分享」（Save and share）\n3. 选择「安装 PlushieWord...」\n\n或者：地址栏右侧通常会出现一个「安装」小图标，点一下也行。\n\n如果 Chrome 说「已安装」但 Dock 里找不到：在地址栏访问 chrome://apps，找到 PlushieWord 右键 →「从 Chrome 中删除」，然后再回来重试。',
  ...
  en: {
    ...
    installIosTip: 'Tap the "⋯" button at the bottom, then tap "Share", choose "View More", scroll down, and select "Add to Home Screen".',
    installIosChromeTip: 'Tap the Share button in the top-right, choose "View More", scroll down, then select "Add to Home Screen".',
    installAndroidTip: 'Tap the "⋮" menu in the top-right of Chrome, then choose "Install app" or "Add to Home screen".\n\n(If this is your first visit, please refresh the page and try again.)',
    installDesktopTip: 'It might already be installed — check your Dock / desktop.\n\nIf not: click the install icon at the right side of the address bar, or use the browser menu → "Install app".',
  ...
  ja: {
    ...
    installIosTip: '下部の「⋯」ボタンをタップし、続けて「共有」をタップし、「もっと見る」を選んで下にスクロールし、「ホーム画面に追加」を選択してください。',
    installIosChromeTip: '右上の「共有」ボタンをタップし、「もっと見る」を選んで下にスクロールし、「ホーム画面に追加」を選択してください。',
    installAndroidTip: 'Chrome 右上の「⋮」メニューをタップし、「アプリをインストール」または「ホーム画面に追加」を選択してください。\n\n（初回アクセスの場合は、ページを再読み込みしてからもう一度お試しください。）',
    installDesktopTip: 'Dock やデスクトップに既にインストールされていないか確認してみてください。\n\nまだの場合：アドレスバー右側の「インストール」アイコン、またはブラウザ右上のメニュー →「アプリをインストール」をクリックしてください。',
