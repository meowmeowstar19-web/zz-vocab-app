...
export const UI_TEXT = {
  zh: {
    ...
    installIosTip: '点击 Safari 底部的「⋯」按钮，点击「分享」，选择「View More」，下滑找到「添加到主屏幕」。',
    installIosChromeTip: '点击 Chrome 右上角的「分享」按钮，选择「View More」，下滑找到「添加到主屏幕」。',
    ...
    installDesktopTip: '点地址栏右侧的「安装」小图标，或者打开右上角「⋮」菜单 →「投放、保存和分享」→「安装 PlushieWord...」。\n\n如果地址栏没有安装图标、菜单里也没有安装选项（Chrome 还在准备中）：刷新一下页面再点这里试试。\n\n如果之前装过又删了：在地址栏输入 chrome://apps，找到 PlushieWord 右键 →「从 Chrome 中删除」，再回来重试。',
  ...
  en: {
    ...
    installIosTip: 'Tap the "⋯" button at the bottom of Safari, tap "Share", choose "View More", scroll down and select "Add to Home Screen".',
    installIosChromeTip: 'Tap the Share button at the top-right of Chrome, choose "View More", scroll down and select "Add to Home Screen".',
    ...
    installDesktopTip: 'Click the install icon at the right side of the address bar, or open the "⋮" menu → "Cast, save, and share" → "Install PlushieWord...".\n\nIf there\'s no install icon and no install option (Chrome is still warming up): refresh the page and click this button again.\n\nIf you previously installed and deleted it: visit chrome://apps, right-click PlushieWord → "Remove from Chrome", then try again.',
  ...
  ja: {
    ...
    installIosTip: 'Safari 下部の「⋯」ボタンをタップし、「共有」をタップし、「もっと見る」を選び、下にスクロールして「ホーム画面に追加」を選んでください。',
    installIosChromeTip: 'Chrome 右上の「共有」ボタンをタップし、「もっと見る」を選び、下にスクロールして「ホーム画面に追加」を選んでください。',
    ...
    installDesktopTip: 'アドレスバー右側の「インストール」アイコン、または右上「⋮」メニュー →「キャスト、保存、共有」→「PlushieWord をインストール...」をクリックしてください。\n\nインストールアイコンも選択肢も見当たらない場合（Chrome の準備中）：ページを再読み込みしてから、もう一度このボタンをクリックしてください。\n\n以前インストール後に削除した場合：アドレスバーに chrome://apps を入力し、PlushieWord を右クリック →「Chrome から削除」してから、もう一度お試しください。',
