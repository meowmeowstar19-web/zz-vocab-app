...
function writeDevPhrasesJs(devPhrases, devCats, words) {
  ...
  out += `// 英语词组 + 中文翻译 alone; 英语例句 optional; no audio/phonetics/word images.\n`;
  out += `// Format: [english, chinese, category, sentence]\n\n`;
  ...
  for (const p of ordered) {
    ...
    out += `  [${[jsStr(p.en), jsStr(p.zh), jsStr(p.category), jsStr(p.sentence || '')].join(', ')}],\n`;
  }
  out += `];\n\n`;
  ...
  out += `}\n\n`;
  out += `export const devPhrases = raw.map(([en, zh, category, sentence]) => ({\n`;
  out += `  id: makeId(en),\n`;
  out += `  en, zh, category,\n`;
  out += `  img: null,\n`;
  out += `  level: 'dev',\n`;
  out += `  sentence: sentence || '', sentenceZh: '',\n`;
