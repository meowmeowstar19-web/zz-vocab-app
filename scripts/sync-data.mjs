...
function readVocab() {
  ...
  for (const row of rows) {
    ...
    // Only include words that have an example sentence
    if (!sentence) {
      continue;
    }
  ...
  log.ok(`Parsed ${words.length} vocab entries (skipped rows without example sentences)`);
  return { wb, words };
}
...
async function main() {
  ...
  const { wb, words: allWords } = readVocab();
